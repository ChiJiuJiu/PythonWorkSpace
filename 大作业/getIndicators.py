...
def getIndicators(data):
    ...
    for game in data:
        ...
        list.append({"name":name, "theme":",".join(theme_list), "dev":",".join(dev), "publisher":",".join(publisher), "price":"".join(price_list), "platform":platform, "category":category, "is_support_chinese":is_support, "top":top})
...
def writeToCsv(data):
    try:
        ...
        for game in data:
            print(game)
            writer.writerow([game['name'], game['theme'], game['dev'], game['publisher'], game['price'], game['platform'], game['category'], game["is_support_chinese"], game['top']])
    except IOError as e:
        print(e)
