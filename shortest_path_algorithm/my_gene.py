...
class Guessor(object):
    ...
    def guess(self):
        ...
        self.generator(self.init_candidates_number)
        self.iteration_times=0
        while True:
            ...
            #mutation
            self.mutation(self.mutating_rate)  ##突变
            #cross_over
    ...
    def cross_over(self,possibility=0.0005,window_size=0.3):
        ...
        for i in range(len(cross_over_pairs)):
            ...
            if start_pos==end_pos:
                piece0=self.candidates[cross_over_pairs[i][0]][start_pos]   ##两个基因位点
                piece1=self.candidates[cross_over_pairs[i][1]][start_pos]
            else:
                if end_pos==self.target_len-1:
                    piece0=self.candidates[cross_over_pairs[i][0]][start_pos:]  ##一直到结尾
                    piece1=self.candidates[cross_over_pairs[i][1]][start_pos:]
                else:
                    piece0=self.candidates[cross_over_pairs[i][0]][start_pos:end_pos+1]  ##中间截取
    ...
    def save_stats(self,dir='datas/',plot=False):
        import pandas as pd
...
if __name__=="__main__":
    t=TargetString('hello world liuguodong')
    g=Guessor(t,20000)
    g.guess()
    g.plot_stats()
