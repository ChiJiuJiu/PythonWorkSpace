...
class  Graph_DG:
    ...
    def  dijkstra(self,begin):
        ...
        for i  in  range(self.vexnum):
            ...
            tmp_dis.value=self.arc[begin-1][i]
        ...
        while count<self.vexnum:
            ...
            cur_min_index=0
            for i in  range(self.vexnum):
                if not self.dis[i].visit and self.dis[i].value<cur_min:
                    cur_min=self.dis[i].value
                    cur_min_index=i

            self.dis[cur_min_index].visit=True
            count+=1

            for j  in  range(self.vexnum):
                if  not self.dis[j].visit and self.arc[cur_min_index][j]!=MAX_VALUE and \
                    self.dis[cur_min_index].value+self.arc[cur_min_index][j]<self.dis[j].value:
                    self.dis[j].value=self.dis[cur_min_index].value+self.arc[cur_min_index][j]
                    self.dis[j].path='{0}-->v{1}'.format(self.dis[cur_min_index].path,j+1)
